...
console.log("\nEq. 55–62 — VEV conservation (r-based interpolation):");
const vev_rh = vevBreakdown(R_H);
const vev_ra = vevBreakdown(R_A);
const vev_sm = vevBreakdown(4.0);
...
assert("v²+h² = 246² at r=4", near(vev_sm.v**2 + vev_sm.h**2, VEV**2, 1),
...
assert("v dominates at r=4 (SM exterior)", vev_sm.v > vev_sm.h,
  `v=${vev_sm.v.toFixed(1)} h=${vev_sm.h.toFixed(1)}`);
assert("v ≈ 246 at r=4", near(vev_sm.v, 246, 5),
  `got ${vev_sm.v.toFixed(1)}`);
